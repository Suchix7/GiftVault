    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
      ...
      >
        ...
        <p className="text-gray-500 text-sm mb-8 leading-relaxed font-medium">
          You are about to permanently purge <span className="text-black font-bold">"{entityName}"</span>. This protocol cannot be reversed.
        </p>
  ...
  if (loading)
    return (
      <div className="h-[60vh] flex items-center justify-center text-[10px] font-black uppercase tracking-widest text-gray-400">
        <div className="w-8 h-8 border-4 border-gray-100 border-t-black rounded-full animate-spin mr-3" /> Syncing Ledger...
      </div>
    );
  if (error)
    return (
    ...
    <div className="space-y-10 pb-20">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium text-sm">System-wide biometric and commercial metrics.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
        <StatCard
          ...
          trend
        />
        <StatCard title="Platform Customers" value={totalRegularUsers} sub="Wallet Holders" icon={Activity} />
        <StatCard title="Root Admins" value={totalAdmins} sub="Security Clearance" icon={ShieldCheck} />
        <StatCard title="Total Network" value={users.length} sub="Synchronized Nodes" icon={TrendingUp} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white border border-gray-100 rounded-[2.5rem] p-8 shadow-sm">
          <h3 className="text-xs font-black uppercase tracking-widest mb-8 text-gray-400">Node Distribution</h3>
        ...
        <div className="bg-white border border-gray-100 rounded-[2.5rem] p-8 shadow-sm">
          <h3 className="text-xs font-black uppercase tracking-widest mb-8 text-gray-400">Vendor Verification</h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={[
      ...
      <div className="space-y-8 pb-20">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            ...
            <p className="text-gray-500 font-medium mt-1">Registry of all authorized {type}.</p>
          </div>
          <button
            onClick={() => {
              setCreatingUserType(
            ...
            <table className="min-w-full divide-y divide-gray-50">
              <thead className="bg-gray-50/50">
                <tr className="text-[10px] font-black uppercase tracking-widest text-gray-400">
                  ...
                  {type === "vendors" && <th className="px-8 py-5 text-left">Clearance</th>}
                  <th className="px-8 py-5 text-left hidden sm:table-cell">Genesis Date</th>
                  ...
                  <tr key={user._id} className="group hover:bg-gray-50/30 transition-all">
                    ...
                    <td className="px-8 py-4 text-right">
                      <div className="flex justify-end gap-1 opacity-100 transition-opacity">
                        {type === "vendors" && (
                          <button
                            onClick={() => toggleApprovalStatus(user._id, !user.isApproved)}
    ...
    <div className="flex-1 overflow-y-auto relative scroll-smooth bg-[#FBFBFB] pt-20 lg:pt-0 pb-28 lg:pb-0">
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-8 lg:p-12 relative z-10">
        <AnimatePresence mode="wait">
          ...
          >
            {currentPage === "dashboard" && <UserList users={users} loading={loading} error={error} />}
            {currentPage === "vendors" && <DataRegistry type="vendors" title="Vendor Management" />}
            {currentPage === "customers" && <DataRegistry type="customers" title="Customer Directory" />}
            {currentPage === "admins" && <DataRegistry type="admins" title="Admin Governance" />}
          </motion.div>
        </AnimatePresence>
      </div>
